...
def test_repo_path():
    ...
    context_kit_dir = Path(temp_dir) / ".context-kit"
    context_kit_dir.mkdir()
    (context_kit_dir / "spec-log").mkdir()

    # Create contexts directory structure (where entities are actually stored)
    ...
    # Create sample feature entity as YAML
    import yaml
    ...
    feature_data = {
        ...
        "status": "planning",
        "objective": "User authentication system with JWT tokens",
    }
    with open(contexts_dir / "features" / "feature-auth.yaml", "w") as f:
        yaml.dump(feature_data, f)

    # Create sample user story entity
    userstory_data = {
        "id": "component-login-form",
        "_type": "component",
        "title": "Login Form Component",
        "status": "todo",
        "related_to": ["feature-auth"],
    }
    with open(contexts_dir / "userstories" / "component-login-form.yaml", "w") as f:
...
class TestInspectionWorkflow:
    ...
    def test_inspect_repository(self, test_repo_path):
        ...
        response = client.post("/context/inspect", json={"repo_path": test_repo_path, "depth": 2})
    ...
    def test_inspect_with_type_filter(self, test_repo_path):
        ...
        response = client.post(
            "/context/inspect",
            json={"repo_path": test_repo_path, "include_types": ["feature"], "depth": 1},
        )
    ...
    def test_inspect_invalid_path(self):
        ...
        response = client.post(
            "/context/inspect", json={"repo_path": "/nonexistent/path", "depth": 2}
        )
...
class TestSpecGenerationWorkflow:
    ...
    def test_generate_spec_invalid_repo(self):
        ...
        response = client.post(
            "/spec/generate",
            json={"repo_path": "/nonexistent/path", "entity_ids": [], "user_prompt": "Test prompt"},
        )
    ...
    def test_generate_spec_missing_prompt(self, test_repo_path):
        ...
        response = client.post(
            "/spec/generate",
            json={"repo_path": test_repo_path, "entity_ids": ["feature-auth"], "user_prompt": ""},
        )
...
class TestCompleteE2EPipeline:
    ...
    def test_full_pipeline_flow(self, test_repo_path):
        ...
        inspect_response = client.post(
            "/context/inspect", json={"repo_path": test_repo_path, "depth": 2}
        )
...
class TestSpecLogWorkflow:
    ...
    def test_filter_spec_logs_by_operation(self, test_repo_path):
        ...
        response = client.get(f"/spec-log/list?repo_path={test_repo_path}&operation=spec-generate")
...
class TestErrorHandling:
    ...
    def test_malformed_json(self):
        ...
        response = client.post(
            "/spec/generate", data="{ invalid json", headers={"Content-Type": "application/json"}
        )
        assert response.status_code == 422
