...
const healthBannerClass = computed(() => {
  if (isUnhealthy.value) return 'bg-error-container border-outline text-on-error-container';
  if (isDegraded.value) return 'bg-warning-container border-outline text-on-warning-container';
  return 'bg-surface-variant border-outline text-on-surface-variant';
});
const healthIconClass = computed(() => {
  if (isUnhealthy.value) return 'text-error';
  if (isDegraded.value) return 'text-warning';
    ...
    <Transition name="slide-down">
      <div v-if="showHealthBanner" class="px-4 py-3 border-b flex items-center gap-3" :class="healthBannerClass">
        ...
        <button
          ...
          :class="isUnhealthy.value ? 'bg-error text-on-error hover:bg-error-hover' : 'bg-warning text-on-warning hover:bg-warning-hover'"
